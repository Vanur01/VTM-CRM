...
const TasksPipeline = () => {
  ...
  useEffect(() => {
    if (user?.companyId) {
      fetchManagerUsersTasks(user.companyId, {
        page: currentPage,
        limit: itemsPerPage
      });
    }
  }, [fetchManagerUsersTasks, currentPage, itemsPerPage, user?.companyId]);
  ...
  const handleDrop = async (e: React.DragEvent, targetStageId: string) => {
    ...
    // Check if user is manager and prevent status change
    if (user?.role === 'manager') {
      setWarningMessage("You cannot change the task status as a manager.");
    ...
    try {
      ...
      if (user?.companyId) {
        await fetchManagerUsersTasks(user.companyId);
      }
    } catch (error) {
  ...
  const handleTaskEdit = (task: Task) => {
    if (user?.role === 'manager') {
      setWarningMessage("You cannot edit tasks as a manager.");
      setShowWarningDialog(true);
      return;
    }
  ...
  const handleTaskDelete = (task: Task) => {
    if (user?.role === 'manager') {
      setWarningMessage("You cannot delete tasks as a manager.");
      setShowWarningDialog(true);
      return;
    }
