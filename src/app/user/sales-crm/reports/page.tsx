...
const ReportsPage = () => {
  ...
  const { userReport, isLoading, error, fetchUserReports } = useReportsStore();
      ...
      <div className="h-full overflow-y-auto px-4 py-4 rounded-xl custom-scrollbar space-y-6">
        ...
        {isLoading ? (
          <div className="flex justify-center items-center py-8">
            <CircularProgress />
            <p className="ml-3 text-gray-600">Loading user reports...</p>
          </div>
        ) : error ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <StatCard
              title="Total Leads"
              value={userReport.totalLeads}
              bgColor="bg-blue-50"
              textColor="text-blue-700"
            />
            <StatCard
              title="Converted Leads"
              value={userReport.convertedLeads}
              bgColor="bg-green-50"
              textColor="text-green-700"
            />
            <StatCard
              title="Not Converted Leads"
              value={userReport.notConvertedLeads}
              bgColor="bg-yellow-50"
              textColor="text-yellow-700"
            />
            <StatCard
              title="Total Meetings"
              value={userReport.totalMeetings}
              bgColor="bg-purple-50"
              textColor="text-purple-700"
            />
            <StatCard
              title="Total Calls"
              value={userReport.totalCalls}
              bgColor="bg-indigo-50"
              textColor="text-indigo-700"
            />
            <StatCard
              title="Total Tasks"
              value={userReport.totalTasks}
              bgColor="bg-pink-50"
